...
If the note relates to existing notes, add `[[links]]` to connect them. Search with qmd only when its collection is confirmed not to index `credenciais/`; otherwise use targeted `rg` over the relevant content folder.
...
7. Update the qmd index only when its collection is confirmed safe for the current vault. Otherwise skip it and say why.
